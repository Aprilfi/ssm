...
function BSForm(eleConfigs) {
        ...
        function SetDefaultValue(eleConfig, model) {
            ...
            if (eleConfig && ele && ele['name']) {
                ...
                if (key !== undefined && eleValue !== undefined) {
                    if (ele['type'] === 'text' || ele['type'] === 'datetime' || ele['type'] === 'search' || ele['type'] === 'textarea') {
                        $("#" + key, global.Fn.$(cur.containerID)).val(eleValue || '');
                    }
                    else if (ele['type'] === 'select') {
                        try {
                            var s = selectItem("#" + key, global.Fn.$(cur.containerID));
    ...
    * return this;
    ...
    function _GenerateFormElement($target, eleConfig) {
        ...
        if (eleConfig.group) {
            _SetCommentAttr($target, eleConfig.group);
        }
        if (feType === "select") {
